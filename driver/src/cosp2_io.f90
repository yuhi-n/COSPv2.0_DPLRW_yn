...
contains
  ...
  subroutine write_cosp2_output(Npoints, Ncolumns, Nlevels, Ninst_rttov, lev, lon, lat, cospOUT, outFileName)
    ...
    if (associated(cospOUT%dplrw_Z)) then
       ! axes
       status = nf90_def_var(fileID,"Nlvtemp",nf90_float,(/dimID(19)/),varID(150))
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       ...
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_def_var(fileID,"NlvdBZe",nf90_float,(/dimID(20)/),varID(151))
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(151),"long_name","dBZe axis for DPLRW CFED")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_put_att(fileID,varID(151),"units",        "dBZe")
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_def_var(fileID,"Nlvdplr",nf90_float,(/dimID(21)/),varID(152))
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       ...
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
       status = nf90_def_var(fileID,"Nlvspwd",nf90_float,(/dimID(22)/),varID(153))
       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status))
